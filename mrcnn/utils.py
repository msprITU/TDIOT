...
def bb_intersection_over_union(boxA, boxB):
    """
        ...
        boxA: [x1, y1, x2, y2] (top left and bottom right corners)
    """
...
def box_convert_xywh_to_corners(box):
...
def sample_bbox(seed, stdev_coord, stdev_scale, count, canvas_shape):
    # sample format = [x_part, y_part, w_part, h_part]
    x, y, w, h = seed
    # particles = np.zeros((1,400,4)) # ilk framede particlelar hep 0 olacak
    bboxes = np.zeros((1, count, 4))

    for i in range(count):
        x_sample = random.gauss(x,stdev_coord)
        if x_sample < 0:
            print("zero x sample")
            x_sample = 0
        elif x_sample > canvas_shape[0]:
            print("x sample {} greater than canvas {}".format(x_sample,canvas_shape[0]))
            x_sample = canvas_shape[0]
        bboxes[0, i, 0] = x_sample

        y_sample = random.gauss(y,stdev_coord)
        if y_sample < 0:
            print("zero y sample")
            y_sample = 0
        elif y_sample > canvas_shape[1]:
            print("y sample {} greater than canvas {}".format(y_sample, canvas_shape[1]))
            y_sample = canvas_shape[1]
        bboxes[0, i, 1] = y_sample

        w_sample = random.gauss(w, stdev_scale)
        if w_sample < 0:
            print("zero w sample")
            w_sample = 0
        elif x_sample + w_sample > canvas_shape[0]:
            print("w sample out of bounds: sum: {} shape: {}".format(x_sample + w_sample,canvas_shape[0]))
            w_sample = canvas_shape[0] - x_sample
            print("w sample: {}".format(w_sample))
        bboxes[0, i, 2] = w_sample

        h_sample = random.gauss(h, stdev_scale)
        if h_sample < 0:
            print("zero h sample")
            h_sample = 0
        elif y_sample + h_sample > canvas_shape[1]:
            print("h sample out of bounds: sum: {} shape: {}".format(y_sample + h_sample, canvas_shape[1]))
            h_sample = canvas_shape[1] - y_sample
            print("h sample: {}".format(h_sample))
        bboxes[0, i, 3] = h_sample



        # bboxes[0, i, 0] = abs(random.gauss(x, stdev))
        # bboxes[0, i, 1] = abs(random.gauss(y, stdev))
        # bboxes[0, i, 2] = abs(random.gauss(w, stdev))
        # bboxes[0, i, 3] = abs(random.gauss(h, stdev))

    return bboxes

def calculate_average_motion(bbox_list):
    motion_x = 0
    motion_y = 0
    change_x = 0
    change_y = 0
    list_size = len(bbox_list)

    if list_size == 0:
        return motion_x, motion_y, change_x, change_y

    for i in range(1,list_size):
        mx,my,cx,cy = calculate_motion(bbox_list[i-1],bbox_list[i])
        motion_x += mx
        motion_y += my
        change_x += cx
        change_y += cy

    return [motion_x/list_size, motion_y/list_size, change_x/list_size, change_y/list_size]


def calculate_motion(prev, current):
    # sample format = [x_part, y_part, w_part, h_part]
    ...
    curr_center_x, curr_center_y = curr_x + curr_w / 2, curr_y + curr_h / 2
    # particles = np.zeros((1,400,4)) # ilk framede particlelar hep 0 olacak
    x_motion = curr_center_x - prev_center_x
    y_motion = curr_center_y - prev_center_y
    x_scale = curr_w - prev_w
    y_scale = curr_h - prev_h

    return [x_motion, y_motion, x_scale, y_scale]
...
def apply_motion(bbox, motion):
    x, y, w, h = bbox
    x_motion, y_motion, x_scale, y_scale = motion
    x_new = x + x_motion
    y_new = y + y_motion
    w_new = w + x_scale
    h_new = h + y_scale
...
def calculate_average_bbox(bbox_list):
    x_cum = 0
    y_cum = 0
    w_cum = 0
    h_cum = 0
    ...
    return [x_cum/bbox_cnt, y_cum/bbox_cnt, w_cum/bbox_cnt, h_cum/bbox_cnt]

def sample_bbox_from_motion_model(seed, motion, stdev_coord, stdev_scale, count, canvas_shape):
    """
    TODO: Denenmedi
    :param seed:
    :param motion:
    ...
    # for i in range(count):
    #     bboxes[0, i, 0] = abs(random.gauss(x_new, stdev))
    #     bboxes[0, i, 1] = abs(random.gauss(y_new, stdev))
    #     bboxes[0, i, 2] = abs(random.gauss(w_new, stdev))
    #     bboxes[0, i, 3] = abs(random.gauss(h_new, stdev))

    return sample_bbox([x_new, y_new, w_new, h_new],stdev_coord,stdev_scale,count,canvas_shape)
    # return bboxes
...
def bbox_distance(bbox1, bbox2):
    ...
    dist = math.sqrt(math.pow(center_x_1-center_x_2,2)+math.pow(center_y_1-center_y_2,2))
